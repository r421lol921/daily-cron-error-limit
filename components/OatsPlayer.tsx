...
// Short pop sound as base64 audio (click/pop)
const POP_SOUND = 'data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAA...'
...
export default function OatsPlayer({ oat, currentUserId, isActive, onViewCounted }: Props) {
  ...
  // Pause video when comments drawer opens
  useEffect(() => {
    const v = videoRef.current
    if (!v) return
    if (showComments) {
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden select-none">
      ...
      <div className="absolute right-3 bottom-20 flex flex-col items-center gap-4 z-20">
        ...
        {/* Like — heart icon, pop sound, bounce anim */}
        <div className="relative group/like flex flex-col items-center">
          ...
          >
            <div className={`w-11 h-11 flex items-center justify-center transition-transform ${liked ? 'text-red-500' : 'text-white'} ${likeAnim ? 'scale-150' : 'active:scale-125'}`}>
              {/* Outlined heart matching reference */}
          ...
          <span className="absolute right-full mr-2 top-3 pointer-events-none opacity-0 group-hover/like:opacity-100 transition-opacity bg-black/70 text-white text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">Like</span>
        </div>

        {/* Comments — speech bubble */}
        <div className="relative group/cmt flex flex-col items-center">
          <button
          ...
          <span className="absolute right-full mr-2 top-3 pointer-events-none opacity-0 group-hover/cmt:opacity-100 transition-opacity bg-black/70 text-white text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">Comment</span>
        </div>
        ...
        <div className="relative group/bm flex flex-col items-center">
          ...
          <span className="absolute right-full mr-2 top-3 pointer-events-none opacity-0 group-hover/bm:opacity-100 transition-opacity bg-black/70 text-white text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">Bookmark</span>
        </div>

        {/* Share */}
        <div className="relative group/sh flex flex-col items-center">
          <button
          ...
          <span className="absolute right-full mr-2 top-3 pointer-events-none opacity-0 group-hover/sh:opacity-100 transition-opacity bg-black/70 text-white text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">Share</span>
        </div>
