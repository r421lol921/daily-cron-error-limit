...
export default function OatsClient({ initialOats, currentUserId }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Up/down chevrons for desktop */}
      {activeIndex > 0 && (
        <button
          onClick={() => goTo(activeIndex - 1, 'down')}
          className="absolute top-16 left-1/2 -translate-x-1/2 z-30 bg-black/30 backdrop-blur-sm rounded-full p-1.5 hidden sm:flex"
          aria-label="Previous oat"
        >
      ...
      {activeIndex < oats.length - 1 && (
        <button
          ...
          className="absolute bottom-14 left-1/2 -translate-x-1/2 z-30 bg-black/30 backdrop-blur-sm rounded-full p-1.5 hidden sm:flex"
          aria-label="Next oat"
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth="2.5">
